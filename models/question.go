...
func GetUserQuestionsList(userId int64) ([]Question, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&question.ID, &question.Title, &question.Body, &question.UserID, &question.CreatedAt, &question.UpVotes, &question.UpVotes, &question.DownVotes, &question.Status)
		if err != nil {
			return nil, err
		}
